...
import { contractAbi as abi } from "@/app/abi";

// TODO: Replace with your deployed address or inject via env
export const SNEAK_PROTOCOL_ADDRESS = process.env
    .NEXT_PUBLIC_SNEAK_PROTOCOL_ADDRESS as `0x${string}` | undefined;
...
export function useGetUserTokens(
    ...
    const enabled = useMemo(
        () => opportunityId !== undefined && side !== undefined && !!user,
        [opportunityId, side, user]
    );
    return useReadContract({
        address,
        abi,
        functionName: "getUserTokens",
        args: [opportunityId as any, side as any, user as any],
        query: { enabled },
    });
}
...
export function useGetOpportunityRiskData(opportunityId?: BigNumberish) {
    ...
    return useReadContract({
        address,
        abi,
        functionName: "getOpportunityRiskData",
        args: [opportunityId as any],
        query: { enabled },
    });
}
...
export function useGetChainHealthData(chainId?: BigNumberish) {
    ...
    return useReadContract({
        address,
        abi,
        functionName: "getChainHealthData",
        args: [chainId as any],
        query: { enabled },
    });
}
...
export function useGetChainRiskAnalysis(chainId?: BigNumberish) {
    ...
    return useReadContract({
        address,
        abi,
        functionName: "getChainRiskAnalysis",
        args: [chainId as any],
        query: { enabled },
    });
}
...
export function useUserChains(user?: `0x${string}`, index?: BigNumberish) {
    ...
    return useReadContract({
        address,
        abi,
        functionName: "userChains",
        args: [user as any, index as any],
        query: { enabled },
    });
}
