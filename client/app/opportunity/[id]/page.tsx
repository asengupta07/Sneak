...
export default function OpportunityDetail() {
    ...
    const fetchOpportunity = async () => {
      try {
        ...
        const mockOpportunity: Opportunity = {
          ...
          description: "This opportunity speculates on Bitcoin's price reaching $100,000 USD by December 31, 2024. The resolution will be based on the closing price of Bitcoin on major exchanges at 11:59 PM UTC on December 31, 2024.",
          category: "Crypto",
          status: "active",
          resolutionCriteria: "Bitcoin price must reach or exceed $100,000 USD on at least one major exchange (Coinbase, Binance, Kraken) at any point before December 31, 2024, 11:59 PM UTC.",
          ...
          banners: ["/api/placeholder/1200/300"],
          isCreator: true,
  ...
  const confirmOutcomeSelection = async () => {
    ...
    try {
      ...
      console.log(`Setting outcome for ${opportunity.id} to ${selectedOutcome}`)
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      console.log(`Trading ${tradingAmount} ETH on ${tradingSide} side`)
      ...
      >
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-300 mb-2">Opportunity not found</h3>
            <p className="text-gray-400 mb-6">The opportunity you're looking for doesn't exist.</p>
            <button
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 py-12 max-w-6xl">
        ...
        <div className="bg-gray-900/50 border border-orange-500/20 rounded-2xl p-8 mb-8">
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-4">
                ...
                {opportunity.isCreator && (
                  <span className="text-orange-400 text-sm font-medium">CREATOR</span>
                )}
                {getOutcomeIcon(opportunity.outcome)}
              </div>
              <h1 className="text-3xl font-bold text-white mb-4">{opportunity.title}</h1>
              <p className="text-lg text-gray-300 leading-relaxed">{opportunity.description}</p>
            </div>
            ...
            <div className="flex items-center space-x-4 mt-6 lg:mt-0">
              ...
              {opportunity.isCreator && (
                <div className="flex space-x-2">
                  <button
                    onClick={() => router.push(`/edit-opportunity/${opportunity.id}`)}
          ...
          <div className="flex items-center space-x-3 mb-6">
            ...
            />
            <div>
              <p className="text-white font-medium">{opportunity.creator.name}</p>
              <p className="text-gray-400 text-sm">{opportunity.creator.address}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-white">{opportunity.currentLiquidity.toFixed(2)} ETH</p>
            </div>
            <div className="text-center">
              <p className="text-gray-400 text-sm">Total Volume</p>
              <p className="text-2xl font-bold text-white">{opportunity.totalVolume.toFixed(1)} ETH</p>
            </div>
            <div className="text-center">
              <p className="text-gray-400 text-sm">Participants</p>
              <p className="text-2xl font-bold text-white">{opportunity.participants}</p>
            </div>
            <div className="text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6">
              ...
              <p className="text-gray-300 leading-relaxed">{opportunity.resolutionCriteria}</p>
            </div>
                ...
                {positions.map((position) => (
                  <div key={position.id} className="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className={`w-3 h-3 rounded-full ${position.side === 'yes' ? 'bg-green-400' : 'bg-red-400'}`}></div>
                      <div>
                        <p className="text-white font-medium">
                          {position.side.toUpperCase()} - {position.amount} ETH
                        </p>
                        <p className="text-gray-400 text-sm">
                          {position.user} • {new Date(position.timestamp).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-white font-semibold">{(position.price * 100).toFixed(1)}%</p>
          ...
          <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="text-sm">{(opportunity.yesPrice * 100).toFixed(1)}%</p>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="text-sm">{(opportunity.noPrice * 100).toFixed(1)}%</p>
                      </div>
                    </button>
                  </div>
            ...
            {opportunity.isCreator && opportunity.status === 'active' && (
              <div className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6">
                <h3 className="text-xl font-semibold text-white mb-4">Creator Actions</h3>
      ...
      {showOutcomeModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-orange-500/20 rounded-xl p-8 max-w-md w-full mx-4">
            <h3 className="text-2xl font-bold text-white mb-4">Confirm Outcome</h3>
            ...
            <p className="text-sm text-gray-400 mb-6">
              This action cannot be undone. All participants will be notified and rewards will be distributed.
            </p>
      ...
      {showTradingModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-orange-500/20 rounded-xl p-8 max-w-md w-full mx-4">
            <h3 className="text-2xl font-bold text-white mb-4">Confirm Trade</h3>
            <div className="bg-gray-800/50 rounded-lg p-4 mb-6">
              <p className="text-gray-300 mb-2">
                <span className="font-semibold">{tradingAmount} ETH</span> on{" "}
                <span className={`font-semibold ${tradingSide === 'yes' ? 'text-green-400' : 'text-red-400'}`}>
                  {tradingSide.toUpperCase()}
                </span>
              </p>
              <p className="text-sm text-gray-400">
                Price: {tradingSide === 'yes' ? (opportunity.yesPrice * 100).toFixed(1) : (opportunity.noPrice * 100).toFixed(1)}%
              </p>
            </div>
            <div className="flex space-x-4">
              <button
