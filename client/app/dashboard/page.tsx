          ...
          <div className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {opportunities
                    .reduce((sum, opp) => sum + opp.totalVolume, 0)
                    .toFixed(1)}{" "}
                  ETH
                </p>
              </div>
...
function ChainCard({
  ...
  const canLiquidate = Boolean((preview as any)?.result?.canLiquidate ?? false);

  return (
    <div className="bg-gray-800/50 rounded-lg p-4 border border-orange-500/10">
      <div className="flex items-center justify-between mb-2">
        <p className="text-white font-semibold">Chain #{String(chainId)}</p>
        <span
        ...
        >
          {(health as any)?.result?.isLiquidationRisk ? "At Risk" : "Healthy"}
        </span>
      </div>
