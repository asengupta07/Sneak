...
export default function OpportunitiesDashboard() {
  ...
  const handleOutcomeSelection = (opportunity: Opportunity, outcome: 'yes' | 'no') => {
  ...
  const confirmOutcomeSelection = async () => {
    ...
    try {
      ...
      console.log(`Setting outcome for ${selectedOpportunity.id} to ${selectedOutcome}`)
      
      // Update local state
      setOpportunities(prev => prev.map(opp => 
        opp.id === selectedOpportunity.id 
          ? { ...opp, outcome: selectedOutcome, status: 'resolved' as const, resolvedAt: new Date().toISOString() }
          : opp
      ))
    ...
    >
      ...
      <div className="container mx-auto px-6 py-12 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-orange-500 mb-2">Opportunities Dashboard</h1>
            <p className="text-lg text-gray-300">Manage and monitor all opportunities</p>
          </div>
          <button
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {opportunities.reduce((sum, opp) => sum + opp.totalVolume, 0).toFixed(1)} ETH
                </p>
              </div>
          ...
          <div className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {opportunities.reduce((sum, opp) => sum + opp.participants, 0)}
                </p>
              </div>
            ...
            <div key={opportunity.id} className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6 hover:border-orange-500/40 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    {opportunity.isCreator && (
                      <span className="text-orange-400 text-xs font-medium">CREATOR</span>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-white font-semibold">{opportunity.currentLiquidity.toFixed(2)} ETH</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs">Volume</p>
                  <p className="text-white font-semibold">{opportunity.totalVolume.toFixed(1)} ETH</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-white font-semibold">{(opportunity.yesPrice * 100).toFixed(1)}%</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex space-x-2">
                  ...
                  {opportunity.isCreator && opportunity.status === 'active' && (
                    <button
                      onClick={() => handleOutcomeSelection(opportunity, 'yes')}
                  ...
                  {opportunity.isCreator && opportunity.status === 'active' && (
                    <button
                      onClick={() => handleOutcomeSelection(opportunity, 'no')}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-orange-500/20 rounded-xl p-8 max-w-md w-full mx-4">
            ...
            <p className="text-gray-300 mb-6">
              Are you sure you want to set the outcome of "{selectedOpportunity.title}" to{" "}
