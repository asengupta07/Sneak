...
const uploadToIPFSFile = async (file: File) => {
  const result = await pinata.upload.public.file(file);
  return `${process.env.NEXT_PUBLIC_IPFS_GATEWAY}/ipfs/${result.cid}`;
}

const uploadToIPFSJSON = async (data: any) => {
...
export default function CreateOpportunity() {
  ...
  const handleFileUpload = async (
    ...
    const uploadedFiles = await Promise.all(newFiles.map(uploadToIPFSFile));
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const metadataUrl = await uploadToIPFSJSON(formDataJson);
