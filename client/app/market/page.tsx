...
export default function MarketPage() {
    ...
    const mockOpportunities: Opportunity[] = [
      {
        ...
        description: "This opportunity speculates on Bitcoin's price reaching $100,000 USD by December 31, 2024.",
        category: "Crypto",
        status: "active",
      ...
      },
      {
        ...
        description: "Speculation on whether OpenAI will publicly release GPT-5 before the end of 2024.",
        category: "Technology",
        status: "active",
      ...
      },
      {
        ...
        description: "Economic speculation on whether the US will enter a technical recession in 2024.",
        category: "Finance",
        status: "active",
        resolutionDate: "2024-12-31T23:59:59Z",
  ...
  }, [opportunities, searchTerm, statusFilter, categoryFilter, sortBy, sortOrder])

  const calculateNewPrice = (currentLiquidity: number, currentPrice: number, tradeAmount: number, side: 'yes' | 'no') => {
    if (side === 'yes') {
  ...
  const handleTrade = async (opportunity: Opportunity) => {
    ...
    try {
      ...
      const newYesPrice = calculateNewPrice(opportunity.yesLiquidity, opportunity.yesPrice, tradeAmount, 'yes')
      const newNoPrice = calculateNewPrice(opportunity.noLiquidity, opportunity.noPrice, tradeAmount, 'no')

      // TODO: Implement actual trading logic with smart contract
      console.log(`Trading ${tradeAmount} USD on ${tradingSide} side for opportunity ${opportunity.id}`)
      console.log(`New YES price: ${newYesPrice.toFixed(3)}, New NO price: ${newNoPrice.toFixed(3)}`)
      ...
      setOpportunities(prev => prev.map(opp => 
        opp.id === opportunity.id 
          ? {
              ...opp,
              currentLiquidity: opp.currentLiquidity + tradeAmount,
              yesLiquidity: tradingSide === 'yes' ? opp.yesLiquidity + tradeAmount : opp.yesLiquidity,
              noLiquidity: tradingSide === 'no' ? opp.noLiquidity + tradeAmount : opp.noLiquidity,
              yesPrice: newYesPrice,
              noPrice: newNoPrice,
              totalVolume: opp.totalVolume + tradeAmount,
              participants: opp.participants + 1
            }
          : opp
      ))
  ...
  const handleChainPosition = async (opportunity: Opportunity) => {
    ...
    try {
      ...
      const maxAllocation = selectedPosition.value * 0.6 // 60% LTV
      const interest = 5 // $5 interest
      const liquidationPrice = chainAmount + interest

      if (chainAmount > maxAllocation) {
        alert(`Maximum allocation is $${maxAllocation.toFixed(2)} (60% of position value)`)
        return
      }

      // TODO: Implement actual chaining logic with smart contract
      console.log(`Chaining ${chainAmount} USD from position ${selectedPosition.opportunityId} to opportunity ${opportunity.id}`)
      console.log(`Liquidation price: $${liquidationPrice}`)

      // Add new chained position
      const newPosition: UserPosition = {
        opportunityId: opportunity.id,
        side: chainingSide,
        amount: chainAmount,
        price: chainingSide === 'yes' ? opportunity.yesPrice : opportunity.noPrice,
        value: chainAmount,
        liquidationPrice: liquidationPrice,
        chainLevel: selectedPosition.chainLevel + 1,
        parentPosition: selectedPosition.opportunityId
      }
  ...
  const categories = ["all", "Technology", "Finance", "Sports", "Politics", "Entertainment", "Science", "Business", "Crypto", "Other"]
      ...
      <div className="container mx-auto px-6 py-12 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-lg text-gray-300">Trade on opportunities and build position chains</p>
          </div>
              ...
              {userPositions.map((position, index) => (
                <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-gray-400">Level {position.chainLevel}</span>
                  ...
                  <p className="text-sm text-gray-400">Liquidation: ${position.liquidationPrice?.toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredOpportunities.map((opportunity) => (
            <div key={opportunity.id} className="bg-gray-900/50 border border-orange-500/20 rounded-xl p-6 hover:border-orange-500/40 transition-colors">
              {/* Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-gray-400 text-sm">{opportunity.category}</span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center p-3 bg-gray-800/50 rounded-lg">
                  ...
                  <p className={`text-lg font-bold ${getPriceChangeColor(opportunity.yesPrice)}`}>
                    {(opportunity.yesPrice * 100).toFixed(1)}%
                  </p>
                  <p className="text-xs text-gray-400">${opportunity.yesLiquidity.toFixed(0)}</p>
                </div>
                <div className="text-center p-3 bg-gray-800/50 rounded-lg">
                  <div className="flex items-center justify-center mb-1">
                  ...
                  <p className={`text-lg font-bold ${getPriceChangeColor(opportunity.noPrice)}`}>
                    {(opportunity.noPrice * 100).toFixed(1)}%
                  </p>
                  <p className="text-xs text-gray-400">${opportunity.noLiquidity.toFixed(0)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                <div>
                  ...
                  <p className="text-white font-semibold">${(opportunity.totalVolume / 1000).toFixed(1)}k</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs">Participants</p>
                  <p className="text-white font-semibold">{opportunity.participants}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs">Liquidity</p>
                  <p className="text-white font-semibold">${(opportunity.currentLiquidity / 1000).toFixed(1)}k</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-800/50 rounded-lg p-4">
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-white font-semibold">
                    {tradingAmount ? (parseFloat(tradingAmount) / (tradingSide === 'yes' ? selectedOpportunity.yesPrice : selectedOpportunity.noPrice)).toFixed(2) : '0'} tokens
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">New Price</span>
                  <span className="text-orange-400 font-semibold">
                    {tradingAmount ? (calculateNewPrice(
                      tradingSide === 'yes' ? selectedOpportunity.yesLiquidity : selectedOpportunity.noLiquidity,
                      tradingSide === 'yes' ? selectedOpportunity.yesPrice : selectedOpportunity.noPrice,
      ...
      {showChainingModal && selectedOpportunity && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-orange-500/20 rounded-xl p-8 max-w-md w-full mx-4">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Select Position to Chain</label>
                <select
                  value={selectedPosition?.opportunityId || ""}
                  onChange={(e) => {
                    const position = userPositions.find(p => p.opportunityId === e.target.value)
                ...
                >
                  ...
                  {userPositions.map((position, index) => (
                    <option key={index} value={position.opportunityId}>
                      Level {position.chainLevel} - ${position.value.toFixed(2)} {position.side.toUpperCase()}
                    </option>
                  ))}
                </select>
              </div>
                ...
                <>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-orange-400 font-semibold">$5.00</span>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Amount to Chain (USD)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                  ...
                  {chainingAmount && (
                    <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4">
                      <div className="flex items-center mb-2">
                        ...
                        <span className="text-yellow-400 font-semibold">Liquidation Warning</span>
                      </div>
                      <p className="text-sm text-gray-300">
                        If the parent position value drops below ${(parseFloat(chainingAmount) + 5).toFixed(2)}, 
                        this chained position will be liquidated.
                      </p>
                    </div>
                  )}
                </>
